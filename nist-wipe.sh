...
# Ensure pv and sha256sum are installed
...
wipe_hdd() {
    ...
    for PASS in {1..3}; do
        ...
        dd if="$INPUT" bs=1M status=none | pv -s $DISK_BYTES | dd of="$DISK" bs=1M conv=notrunc status=none &
        DD_PID=$!
    ...
    if [[ $abort_flag -eq 0 ]]; then
        echo -e "\nVérification complète du disque avec SHA-256 (cela peut prendre du temps)..."
...
for DISK in $SELECTED_DISKS; do
    ...
    # Detect type and apply proper wipe
